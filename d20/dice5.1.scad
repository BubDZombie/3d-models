...
tetraheight = 22; // 4 Sided
cubeheight  = 12; // 6 Sided
octaheight  = 12; // 8 Sided
deltoheight = 14; // 10 Sided
dodeheight  = 16; // 12 Sided
icoheight   = 18; // 20 Sided
...
module cubetext(height) {
    ...
    translate([0, 0, 0.5 * height - 1])
	linear_extrude(height=2)
	text("1", size=height * 0.6, valign="center", halign="center", font=font);

    translate([0, 0, -0.5 * height + 1])
    rotate([0, 180, 180])
    linear_extrude(height=2)
	text("6", size=height * 0.6, valign="center", halign="center", font=font);
...
module dodecatext(height,slope) {
    ...
    translate([0, 0, 0.5 * height - text_depth])
    linear_extrude(height=2)
	text("12", size=text_multiplier * height, valign="center", halign="center", font=font);

    translate([0, 0, -0.5 * height + text_depth])
    rotate([0, 180, 0])
    linear_extrude(height=1)
	text("1", size=text_multiplier * height, valign="center", halign="center", font=font);
...
module drawtetra() {
    ...
    difference() {
        intersection() {
            ...
            if(cut_corners)
            rotate([0, 180, 0])
                tetrahedron(tetraheight*3*0.85);
        }
